...
let _nextId = (() => {
    ...
    return entries.length ? Math.max(...entries.map((e: RciEntry) => e.id)) + 1 : 1
})()
...
export default function RCIContainer() {
    ...
    const handleAdd = (e: RciEntry) => { persist([...entries, { ...e, id: nextId() }]); setShowAdd(false) }
    const handleEdit = (updated: RciEntry) => { persist(entries.map(e => e.id === updated.id ? updated : e)); setEditTarget(null) }
    const handleDelete = (id: number) => { if (!window.confirm('Delete this RCI entry?')) return; persist(entries.filter(e => e.id !== id)) }
    ...
    const handleGenerate = async () => {
        ...
        await generateRciReport(filtered, filterPeriod || filtered[0]?.period_covered || '')
    }
                        ...
                        {filtered.length === 0 ? (
                            <tr>
                                <td colSpan={12} className="text-center py-10 text-muted-foreground text-sm">
                                    {hasFilter ? 'No entries match the current filters.' : 'No RCI entries yet. Click "Add Entry" to begin.'}
                                </td>
                            </tr>
                        ) : (
                            filtered.map((e, idx) => (
