...
export default function AddReceivedSaroDialog({ fundSources, onClose, onSaved }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await efasApi.post('saro/received-saros/', payload)
            onSaved()
            onClose()
        } catch (err: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-card border border-border rounded-xl w-full max-w-lg shadow-2xl">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                    <h2 className="font-gbold text-foreground text-lg">Add Received SARO</h2>
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
                    ...
                    <Field label="SARO No." required>
                        <input value={form.saro_no} onChange={e => set('saro_no', e.target.value)} required className={inp} placeholder="2026-01-0031" />
                    </Field>
