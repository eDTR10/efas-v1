...
const emptyForm = {
    ...
    particulars: '',
    net_amount: '',
    status_of_documents: 'complete',
    fund_cluster: 'mds_regular',
    status_of_ntca: 'with_ntca',
    date_of_ntca_download: '',
    mode_of_payment: 'ada',
    date_paid: '',
    remarks: '',
    reasons_responsible: '',
...
export default function AddDisbursementDialog({ onClose, onSaved }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await efasApi.post('saro/disbursements/', payload)
            onSaved()
        } catch (err: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-card border border-border rounded-xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
                    <h2 className="font-gbold text-foreground text-lg">Add Disbursement Record</h2>
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4 overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Field label="DV Number" required>
                            <input value={form.dv_number} onChange={e => set('dv_number', e.target.value)} required className={inp} placeholder="2026-01-0001" />
                        </Field>
                    </div>

                    <Field label="Claimant" required>
                        <input value={form.claimant} onChange={e => set('claimant', e.target.value)} required className={inp} placeholder="e.g. Abao Edison Et al" />
                    </Field>

                    <Field label="Particular">
                        <textarea value={form.particulars} onChange={e => set('particulars', e.target.value)} rows={3} className={inp} placeholder="Payment for…" />
                    </Field>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Field label="Remarks">
                            <input value={form.remarks} onChange={e => set('remarks', e.target.value)} className={inp} placeholder="e.g. Paid thru ADA" />
                        </Field>
                        <Field label="Reasons/Responsible">
                            <input value={form.reasons_responsible} onChange={e => set('reasons_responsible', e.target.value)} className={inp} placeholder="e.g. Completed" />
                        </Field>
                    </div>
