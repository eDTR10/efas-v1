...
function BulkImportModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
    ...
    const parseRows = () => {
        ...
        for (const line of lines) {
            ...
            result.push({ code, name, status: 'pending' })
        }
        setRows(result)
        setParsed(true)
    }

    const updateRow = (idx: number, field: 'code' | 'name', val: string) => {
    ...
    const handleImport = async () => {
        ...
        for (let i = 0; i < updated.length; i++) {
            ...
            try {
                await efasApi.post('fund-type/', { code: updated[i].code.trim(), name: updated[i].name.trim() })
                updated[i] = { ...updated[i], status: 'success', error: undefined }
            } catch (err: unknown) {
                const data = (err as { response?: { data?: Record<string, string[]> } })?.response?.data
                const msg = data?.code?.[0] ?? data?.name?.[0] ?? data?.detail?.[0] ?? 'Failed'
    ...
    return (
        <Modal title="Bulk Import Fund Types" onClose={onClose} maxWidth="max-w-2xl">
            {!parsed ? (
                <div className="flex flex-col gap-4">
                    <div className="bg-muted/30 border border-border rounded-lg p-3 text-xs text-muted-foreground leading-relaxed">
                        ...
                        column 1 = <strong>Code</strong>, column 2 = <strong>Name</strong>.
                                ...
                                {rows.map((row, idx) => (
                                    <tr key={idx} className={`border-b border-border last:border-0 ${row.status === 'success' ? 'bg-green-50 dark:bg-green-950/20' : row.status === 'error' ? 'bg-destructive/5' : ''}`}>
                                        ...
                                        <td className="px-3 py-1.5">
                                            <input value={row.name} onChange={e => updateRow(idx, 'name', e.target.value)} disabled={row.status === 'success' || importing} className={`${rowInp} w-56`} />
...
export default function FundTypeTab() {
    const [items, setItems] = useState<FundType[]>([])
    ...
    const load = async () => {
        setLoading(true)
        try {
            const res = await efasApi.get('fund-type/')
            setItems(res.data)
        } finally {
            setLoading(false)
        }
    ...
    const openAdd = () => { setEditing(null); setForm({ code: '', name: '' }); setError(''); setShowModal(true) }
    const openEdit = (item: FundType) => { setEditing(item); setForm({ code: item.code, name: item.name }); setError(''); setShowModal(true) }

    const handleSave = async () => {
        if (!form.code.trim() || !form.name.trim()) { setError('All fields are required.'); return }
        setSaving(true); setError('')
        try {
            if (editing) {
                await efasApi.put(`fund-type/${editing.id}/`, form)
            } else {
                await efasApi.post('fund-type/', form)
            }
            setShowModal(false)
            load()
        } catch (e: any) {
    ...
    const handleDelete = async (id: number) => {
        ...
        await efasApi.delete(`fund-type/${id}/`)
        load()
    }
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {showModal && (
                <Modal title={editing ? 'Edit Fund Type' : 'Add Fund Type'} onClose={() => setShowModal(false)}>
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-gmedium text-muted-foreground uppercase tracking-wide">Code</label>
                            <input className={inp} placeholder="e.g. 101" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} />
                        </div>
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-gmedium text-muted-foreground uppercase tracking-wide">Name</label>
                            <input className={inp} placeholder="e.g. General Fund" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                        </div>
