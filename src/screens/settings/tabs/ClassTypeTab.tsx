...
function BulkImportModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
    ...
    const handleImport = async () => {
        ...
        for (let i = 0; i < updated.length; i++) {
            ...
            try {
                await efasApi.post('class-type/', { code: updated[i].code.trim(), name: updated[i].name.trim() })
    ...
    return (
        <Modal title="Bulk Import Class Types" onClose={onClose} maxWidth="max-w-2xl">
            {!parsed ? (
                <div className="flex flex-col gap-4">
                    ...
                    <textarea
                        ...
                        placeholder={"Code\tName\n01\tCapital Outlay\n02\tCurrent Operating Expenditures"}
...
export default function ClassTypeTab() {
    ...
    const load = async () => {
        setLoading(true)
        try {
            const res = await efasApi.get('class-type/')
            setItems(res.data)
        } finally {
            setLoading(false)
        }
    ...
    const openEdit = (item: ClassType) => { setEditing(item); setForm({ code: item.code, name: item.name }); setError(''); setShowModal(true) }
    ...
    const handleSave = async () => {
        ...
        try {
            if (editing) {
                await efasApi.put(`class-type/${editing.id}/`, form)
            } else {
                await efasApi.post('class-type/', form)
            }
            setShowModal(false)
            load()
        } catch (e: any) {
    ...
    const handleDelete = async (id: number) => {
        ...
        await efasApi.delete(`class-type/${id}/`)
        load()
    }
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {showModal && (
                <Modal title={editing ? 'Edit Class Type' : 'Add Class Type'} onClose={() => setShowModal(false)}>
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-gmedium text-muted-foreground uppercase tracking-wide">Code</label>
                            <input className={inp} placeholder="e.g. 01" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} />
                        </div>
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-gmedium text-muted-foreground uppercase tracking-wide">Name</label>
                            <input className={inp} placeholder="e.g. Capital Outlay" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                        </div>
